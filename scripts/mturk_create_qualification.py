...
if __name__ == '__main__':
    if len(sys.argv) not in range(2, 4):
        print('Usage: python3 mturk_create_qualification.py qual_name qualification-test.json|password')
        print('If no qualification test file is specified, then creates a qualification test')
        print('that just asks for the specified password.')
    else:
        qual_name = sys.argv[1]
        if sys.argv[2].endswith('.json'):
            qual_test_fname = sys.argv[2]
            qual_password = None
        else:
            qual_password = sys.argv[2]
        ...
        if qual_test_fname is not None:
            ...
            password_question = Question('password', question_content,
                    AnswerSpecification(FreeTextAnswer()))
            qual_test.append(password_question)
        
        print(mturk_connection.create_qualification_type(qual_name,
